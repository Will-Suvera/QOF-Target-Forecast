...
interface TargetAreaSummaryProps {
  areaKey: TargetAreas;
}

export function TargetAreaSummary({ areaKey }: TargetAreaSummaryProps) {
  const { getAreaData, getAllTargetsForArea } = usePracticeData();
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({});

  const areaData = useMemo(() => getAreaData(areaKey), [areaKey, getAreaData]);
  const targets = useMemo(() => getAllTargetsForArea(areaKey), [areaKey, getAllTargetsForArea]);
  const yearProgress = useMemo(() => getFinancialYearProgress(), []);

  // Initialize accordion state when targets change
  useEffect(() => {
    if (targets.length > 0) {
          ...
          return (
            <div key={code}>
              <div className="flex items-center mb-2">
                ...
                >
                  <ChevronRight
                    className={`w-4 h-4 mr-1 transition-transform ${expandedSections[code] === true ? 'rotate-90' : ''}`}
                  />
                  {code}
                </button>
                <div
